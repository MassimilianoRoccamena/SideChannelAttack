...
import main.base.utils.trainable.base as strategy
from main.base.utils.args import parse_args
from main.base.utils.misc import fetch_config, flatten_config, get_source_files
from main.base.utils.logs import LoggerCollection, HyperParamsLogger
...
def do_training():
    ...
    trainable_class = getattr(strategy, cfg.trainable.classname)
    trainable = trainable_class(cfg)

    dt_string = datetime.now().strftime("%Y%m%d_%H%M%S")
    cfg.log_dir = os.path.join(EXPERIMENTS_PATH, cfg.exp_name, dt_string)

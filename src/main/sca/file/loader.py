...
class TraceLoader(FileLoader):
    '''
    Abstract loader of power traces from  file.
    '''
...
class OurTraceLoader(TraceLoader):
    '''
    Abstract loader from file of power traces measured with our technology.
    '''
    ...
    def __init__(self, file_path):
        '''
        Create new basic traces batch file loader.
        file_path: file path
        '''
    ...
    def validate_trace_indices(self, trace_indices):
        ...
        if np.all(trace_indices < 0) or np.all(trace_indices >= self.num_traces):
            raise IndexError(OurTraceLoader.INVALID_TRACE_INDICES_MSG)
